...
df=df.replace("\t?",31)
# Drop the target column from source if it was included (it was seemingly separated earlier but let's be safe)
# In original code: target=df["class"], source=df.drop(["class"]...)
# Issues with tab characters in column interactions were handled earlier.
